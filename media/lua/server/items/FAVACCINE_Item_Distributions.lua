require "Items/ProceduralDistributions"


-- Empty Syringe --
table.insert(ProceduralDistributions.list["MedicalClinicDrugs"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["MedicalClinicDrugs"].items, 30);
table.insert(ProceduralDistributions.list["MedicalClinicOutfit"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["MedicalClinicOutfit"].items, 30);
table.insert(ProceduralDistributions.list["MedicalStorageDrugs"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["MedicalStorageDrugs"].items, 35);
table.insert(ProceduralDistributions.list["MedicalStorageOutfit"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["MedicalStorageOutfit"].items, 15);
table.insert(ProceduralDistributions.list["MedicalStorageTools"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["MedicalStorageTools"].items, 15);
table.insert(ProceduralDistributions.list["SafehouseMedical"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["SafehouseMedical"].items, 25);
table.insert(ProceduralDistributions.list["ScienceMisc"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["ScienceMisc"].items, 15);
table.insert(ProceduralDistributions.list["TestingLab"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["TestingLab"].items, 15);
table.insert(ProceduralDistributions.list["MedicalClinicTools"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["MedicalClinicTools"].items, 10);
table.insert(ProceduralDistributions.list["StoreShelfMedical"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["StoreShelfMedical"].items, 10);
table.insert(ProceduralDistributions.list["ArmyStorageMedical"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["ArmyStorageMedical"].items, 10);
table.insert(ProceduralDistributions.list["BathroomCabinet"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["BathroomCabinet"].items, 2);
table.insert(ProceduralDistributions.list["BathroomCounter"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["BathroomCounter"].items, 2);
table.insert(ProceduralDistributions.list["BathroomShelf"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["BathroomShelf"].items, 2);
table.insert(ProceduralDistributions.list["HospitalLockers"].items, "FAVACCINE.EmptySyringe");
table.insert(ProceduralDistributions.list["HospitalLockers"].items, 20);

-- Dirty Syringe -- 
table.insert(ProceduralDistributions.list["BinDumpster"].items, "FAVACCINE.DirtySyringe");
table.insert(ProceduralDistributions.list["BinDumpster"].items, 20);
table.insert(ProceduralDistributions.list["BinGeneric"].items, "FAVACCINE.DirtySyringe");
table.insert(ProceduralDistributions.list["BinGeneric"].items, 4);
table.insert(ProceduralDistributions.list["BinBar"].items, "FAVACCINE.DirtySyringe");
table.insert(ProceduralDistributions.list["BinBar"].items, 5);
table.insert(ProceduralDistributions.list["PrisonCellRandom"].items, "FAVACCINE.DirtySyringe");
table.insert(ProceduralDistributions.list["PrisonCellRandom"].items, 4);